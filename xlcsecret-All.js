...
const TG_USER_ID = process.env.TG_USER_ID; //TG id
const HELPURL = process.env.HELPURL;//朱丽娜
//个别参数 懂的自己加
const JOY_FEED_COUNT = process.env.JOY_FEED_COUNT; //宠🐕喂食
const JXNCTOKENS = process.env.JXNCTOKENS; //jingxi农场种子
...
async function changeFiele(content, cookie) {
     ...
     let newContent = content.replace("require('./CookieXLCs.js')", JSON.stringify({ CookieXLC: cookie }));
     
     newContent = newContent.replace(`require("./CookieXLCs.js")`, JSON.stringify({ CookieXLC: cookie }));
...
async function executeOneByOne() {
    ...
    for (var i = 0; i < CookieXLCs.length; i++) {
        console.log(`正在执行第${i + 1}个任务`);
        changeFiele(content, CookieXLCs[i]);
        $.UserName = decodeURIComponent(CookieXLCs[i].match(/pt_pin=(.+?);/) && CookieXLCs[i].match(/pt_pin=(.+?);/)[1])
        $.index = i + 1;
        $.nickName = '';
        message = ''
       console.log(`\n******开始【🐶${$.index}】${$.nickName.slice(-4) || $.UserName.slice(-4)}*********\n`);
...
async function start() {
 ...
 //console.log('SyncUrl'+SyncUrl)
    CookieXLCs = XLC.split("&");
    console.log(`当前共${CookieXLCs.length}个账号需要执行任务`);	
    await downFile();
