...
https:\/\/commontgw6\.reader\.qq\.com\/v7_5_2\/nativepage\/getAcctInfo url script-request-header 

#QQ阅读APP获取视频cookie
https:\/\/eventv36\.reader\.qq\.com\/activity\/pkg11955\/watchVideo url script-request-header 
...
QQreadvideohdArr = [], QQreadvideoheaderVal = '',
  ...
  if ($.isNode()) {
	...
	if (process.env.QQREADAPP_HEADER) {
    CASH = process.env.QQREADAPP_HEADER;
  }
...
const QQreadheaderVal = JSON.stringify($request.headers)
    ...
    $.log(`[${QR}] 获取Cookie: 成功,QQreadheaderVal: ${QQreadheaderVal}`)
    $.msg(QR, `获取cookie: 成功🎉`, ``)
 }
...
const QQreadvideoheaderVal = JSON.stringify($request.headers)
    ...
    $.log(`[${QR}] 获取视频Cookie: 成功,QQreadvideoheaderVal: ${QQreadvideoheaderVal}`)
    $.msg(QR, `获取视频cookie: 成功🎉`, ``)
 }
